...
const before = db.prepare('SELECT COUNT(*) as cnt FROM weekly_reports WHERE userId = ?').get('user-1') as any
console.log('Total reports before sanitize:', before.cnt)

const all = db.prepare('SELECT id, weekStart, weekEnd, totalDuration FROM weekly_reports WHERE userId = ? ORDER BY weekStart').all('user-1') as any[]
console.log('\nAll reports:')
